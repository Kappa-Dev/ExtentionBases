...
let build_base l r model =
  ...
  in
  (pos_ext_base,neg_ext_base)

let rebuild env =
  match env.rule with
    None -> env
  | Some (l,r) -> {env with eb = Some (build_base l r env.model)}
...
let rec process_command env = function
  ...
  | Parser.List ->
     log (String.concat "\n" (Model.list env.model)) ;
     env
  | Parser.Build (l,r) ->
     (try
        log (Printf.sprintf "Trying to build extension basis for rule %s -> %s" l r);
        let env = {env with rule = Some (l,r) ; eb = Some (build_base l r env.model)} in
        output env ;
        env
      with Not_found ->
  ...
  | Parser.Add v ->
     if Lib.StringMap.mem v Node.library then
       ...
       | Some _ -> let env = rebuild {env with model = model} in output env ; env
     else
       begin
  ...
  | Parser.Add_named (lst,v) ->
     ...
     (match env.eb with
        None -> {env with model = Model.add_obs v graph env.model}
      | Some _ -> let env = rebuild {env with model = Model.add_obs v graph env.model} in output env ; env
     )
  | Parser.Load file ->
