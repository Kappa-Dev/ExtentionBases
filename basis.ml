...
module Make (Node:Node.NodeType) =
  (struct
    ...
    let rec progress ext_base actions visited best_inf next_layer todo =
      ...
      in
      (************* DEBUGING INFO ***************)
      let update_best_inf ?(force=true) i (inf_to_k,inf,inf_to_w) m =
        if force || not (Lib.IntMap.mem i m) then
          try
            let (_,inf',_) = Lib.IntMap.find i m in
            if is_below inf' inf ext_base then
              Lib.IntMap.add i (inf_to_k,inf,inf_to_w) m
            else
              m
          with
            Not_found -> Lib.IntMap.add i (inf_to_k,inf,inf_to_w) m
        else
          m
      in
        ...
        let ext_base = Lib.IntSet.fold
                         (fun i ext_base ->
                           if i=w then ext_base
                             ...
                             let _,inf,inf_to_w = try Lib.IntMap.find i best_inf with
                                                    Not_found ->
                                                    failwith
                                                      ("Invariant violation: best_inf not defined for point "^(string_of_int i))
                             in
                             (*if not (mem inf ext_base) then ext_base (*inf has been substituted by another best inf*)
                          else*)
                             let () =
                               if db() then print_string
                                              (blue (Printf.sprintf
